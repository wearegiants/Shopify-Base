
<div class="wallpaper banner banner-medium fs-cell fs-full-all" data-background-options='{"source":"/assets/IMG_1388.jpg"}'></div>


<div class="banner banner-white banner-small hidden">
   <div class="centered centered-full">
      <div class="fs-row">
         <div class="fs-cell fs-all-full">
            <span>New Arrivals</span>
         </div>
      </div>
   </div>
</div>

<div id="index--modules" class="hidden">
   <div class="fs-row">
      <div class="fs-cell fs-lg-6 fs-md-6 fs-sm-3"><a href="#" class="home-module medium"></a></div>
      <div class="fs-cell fs-lg-6 fs-md-6 fs-sm-3">
         <a href="#" class="home-module small">Hello</a>
         <a href="#" class="home-module small">Hello</a>
      </div>
      <div class="fs-cell fs-lg-12 fs-md-6 fs-sm-3"><a href="#" class="home-module medium"></a></div>
   </div>
</div>
